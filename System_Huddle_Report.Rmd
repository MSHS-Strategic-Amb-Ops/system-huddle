...
# scheduling_data_raw <- as.data.frame(readRDS("/nfs/data/Applications/Ambulatory/Data/historical_data.rds")) %>%
#   filter(!is.na(Campus)) %>%
...
# scheduling_data_test <- scheduling_data_raw %>% filter(Appt.Year == "2022")
# saveRDS(scheduling_data_test, "historical_data_test.rds")

hh_data_comp_raw <-  as.data.frame(read_csv("/nfs/data/Applications/Ambulatory/hh_comp.csv"))
# hh_data_msd_raw <-  as.data.frame(read_csv("hh_msd.csv"))
...
hh_data_comp_raw$site<- ifelse(hh_data_comp_raw$site == "MSD", 
                                hh_crosswalk$msd_unit[match(hh_data_comp_raw$msd_unit, hh_crosswalk$Location)], hh_data_comp_raw$site)

# Rename msh_unit
hh_data_comp_raw$site <- ifelse(hh_data_comp_raw$site == "MSH", "MSH- AMBULATORY CARE", hh_data_comp_raw$site)
...
#   # mutate(New.PT3 = case_when(New.PT3 == "TRUE" ~ 'New'
#   #                                  ,TRUE ~ 'Established')) %>%
...
missing_exc_dept <- read_excel("/nfs/data/Applications/Ambulatory/open_encounters_exclusion.xlsx", sheet = "Department")
missing_exc_resource <- read_excel("/nfs/data/Applications/Ambulatory/open_encounters_exclusion.xlsx", sheet = "Resource")
missing_exc_visitType <- read_excel("/nfs/data/Applications/Ambulatory/open_encounters_exclusion.xlsx", sheet = "Visit Type")

# missing_exc_dept <- read_excel(file.choose(), sheet = "Department")
# missing_exc_resource <- read_excel(file.choose(), sheet = "Resource")
# missing_exc_visitType <- read_excel(file.choose(), sheet = "Visit Type")
...
missing_chg_data_trend <- scheduling_data %>%
  ...
  group_by(Campus, Appt.MonthYear, clo.7days) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = clo.7days,
              values_from = total,
              values_fill = 0) %>%
  mutate(total.enc = YES + NO,
         clo.7days.perc = round((YES/(total.enc))*100,0))
...
new_vol_trending <- scheduling_data %>%
  ...
  pivot_wider(names_from = New.PT2,
              values_from = total) %>%
  mutate(total_vol = `TRUE` + `FALSE`,
         new_perc = round((`TRUE`/(total_vol))*100,0))
...
highchart() %>% 
  ...
  hc_subtitle(text = paste0("Excludes Radiology and Rad Onc visits <br>", "Based on visits from ",date_start," to ",date_end), 
              align = "left", style = list(fontFace = "italic", fontSize = "16px", fontFamily = "Calibri")) %>%
  hc_tooltip(shared = TRUE, borderColor = "black")
}) %>% 
  hw_grid(ncol = 2, rowheight = 400) %>% htmltools::browsable()
...
hh_site <- bind_rows(hh_site, network, msw)
hh_site <- hh_site %>%
  arrange(site, Month)
...
closed_7days_mshs <- missing_ytd_data %>%
  filter(enc.status == "CLOSED") %>%
  filter(open.days <=7) %>%
  group_by(Campus) %>%
  summarise(clo.7days = n())
...
open_7days_mshs <- missing_ytd_data %>%
  filter(enc.status == "OPEN") %>%
  filter(open.days >7) %>%
  group_by(Campus) %>%
...
missing_chg_ytd_mshs <- Reduce(function(x, y) merge(x, y, all=TRUE), list(closed_ytd_mshs, closed_3days_mshs, closed_7days_mshs, open_ytd_mshs, open_3days_mshs, open_7days_mshs))
missing_chg_ytd_mshs <- missing_chg_ytd_mshs %>%
  mutate(total.count = clo.count + open.count,
         ...
         closed.7days.perc = percent(clo.7days/total.count,0),
         open.3days.perc = percent(open.3days/open.count,0),
         open.7days.perc = percent(open.7days/open.count,0)) %>%
  dplyr::select(Campus, total.count, closed.3days.perc, closed.7days.perc, open.count, enc.open.time, open.3days.perc, open.7days.perc) 


col_names <- c("Site", paste0(todays_year," YTD Total Encounters"), "% of Encounters Closed <=3 Days", "% of Encounters Closed <=7 Days",
               paste0(todays_year, " YTD Open Encounters"), "Median Open Encounter Time", 
               "% of Open Encounters >3 Days", "% of Open Encounters >7 Days")
...
names(header_above_2) <- paste0("Opened Encounters from ",
                              date_start," to ",missing_chg_cutOff)
...
missing_chg_ytd_mshs %>% 
  select(everything()) %>%
    ...
    #      open.7days.perc = color_tile("lightpink","red")(open.7days.perc)) %>%
...
current_last_vol_mshs <- current_last_vol_mshs %>%
  ...
  select(Appt.DateYear, Visit.Method, weekly_vol, prev_week, perc_diff)
...
current_same_vol_mshs <- current_same_vol_mshs %>%
  arrange(Appt.WeekNum) %>%
  select(Appt.DateYear, Visit.Method, weekly_vol, last_year, perc_diff)
...
current_last_wait_mshs_all_ptType <- time_to_appt_all %>%
  filter(Appt.Made.Year %in% unique(past_4_wks$Appt.Year)) %>%
  filter(Appt.Made.WeekNum %in% unique(past_4_wks$Appt.WeekNum)) %>%
  group_by(Appt.Made.Year, Appt.Made.WeekNum, New.PT3) %>%
...
current_last_wait_mshs_combined <- current_last_wait_mshs_combined %>%
  select(Appt.DateYear, Visit.Method, New.PT3, days) %>%
  arrange(Appt.DateYear, New.PT3) %>%
  arrange(factor(Visit.Method, levels = c("Total","IN PERSON","TELEHEALTH")))


current_last_wait_mshs_combined <- current_last_wait_mshs_combined %>%
  pivot_wider(
    names_from = Visit.Method:New.PT3,
    values_from = days
  )
...
current_last_new_mshs <- time_to_appt_all %>%
  ...
  filter(Appt.Made.WeekNum %in% unique(past_4_wks$Appt.WeekNum)) %>%
  mutate(within.14 = ifelse(Wait.Time <= 14, "Yes","No")) %>%
  group_by(Appt.Made.Year, Appt.Made.WeekNum, Visit.Method, New.PT3, within.14) %>%
  summarise(count = n()) %>%
  rename(Appt.Year = Appt.Made.Year,
         Appt.WeekNum = Appt.Made.WeekNum)
...
current_last_new_mshs_total <- current_last_new_mshs %>%
  arrange(Appt.DateYear) %>%
  group_by(Appt.DateYear, New.PT3, within.14) %>%
...
#   arrange(Appt.WeekNum) %>%
#   select(Appt.DateYear, Visit.Method, Arrived, `No Show`, noShow_rate)
#   
# 
...
hh_mshs <- hh_mshs %>%
  ...
  select(Appt.DateYear, role, perc_comp, total_obs)
...
closed_7days_site <- missing_ytd_data %>%
  filter(Campus == site) %>%
  filter(enc.status == "CLOSED") %>%
  filter(open.days <=7) %>%
  group_by(Campus) %>%
...
open_7days_site <- missing_ytd_data %>%
  filter(Campus == site) %>%
  filter(enc.status == "OPEN") %>%
  filter(open.days >7) %>%
  group_by(Campus) %>%
...
missing_chg_ytd_site <- Reduce(function(x, y) merge(x, y, all=TRUE), list(closed_ytd_site, closed_3days_site, closed_7days_site, open_ytd_site, open_3days_site, open_7days_site))
...
closed_7days_dept <- missing_ytd_data %>%
  filter(Campus == site) %>%
  filter(enc.status == "CLOSED") %>%
  filter(open.days <=7) %>%
  group_by(Campus.Specialty, Department) %>%
...
open_7days_dept <- missing_ytd_data %>%
  filter(Campus == site) %>%
  filter(ENC_CLOSED_CHARGE_STATUS == "OPEN") %>%
  filter(open.days >7) %>%
  group_by(Campus.Specialty, Department) %>%
...
missing_chg_ytd_dept <- Reduce(function(x, y) merge(x, y, all=TRUE), list(closed_ytd_dept, closed_3days_dept, closed_7days_dept, open_ytd_dept, open_3days_dept, open_7days_dept))
...
missing_chg_ytd_dept <- missing_chg_ytd_dept %>%
  mutate(total.count = clo.count + open.count,
         ...
         open.3days.perc = percent(open.3days/open.count,0),
         open.7days.perc = percent(open.7days/open.count,0)) %>%
  dplyr::select(Campus.Specialty, Department, total.count, closed.3days.perc, closed.7days.perc, open.count, enc.open.time, open.3days.perc, open.7days.perc) %>%
   mutate_all(., ~replace(., is.na(.), 0))


col_names <- c("Department",paste0(todays_year," YTD Total Encounters"), "% of Encounters Closed <=3 Days", "% of Encounters Closed <=7 Days",
               paste0(todays_year, " YTD Open Encounters"), "Median Open Encounter Time", 
               "% of Open Encounters >3 Days", "% of Open Encounters >7 Days")
...
current_last_vol_dept <- current_last_vol_dept %>%
  arrange(Appt.WeekNum) %>%
  select(Campus.Specialty, Appt.DateYear, Visit.Method, weekly_vol, prev_week, perc_diff)
...
current_same_vol_dept <- current_same_vol_dept %>%
  arrange(Appt.WeekNum) %>%
  select(Campus.Specialty, Appt.DateYear, Visit.Method, weekly_vol, last_year, perc_diff)
...
# time_to_appt_all_site <- time_to_appt_all %>% filter(Campus == site)
  ...
  current_last_wait_site_all_all <- time_to_appt_all %>%
    ...
    filter(Appt.Made.Year %in% unique(past_4_wks$Appt.Year)) %>%
    filter(Appt.Made.WeekNum %in% unique(past_4_wks$Appt.WeekNum)) %>%
    group_by(Appt.Year, Appt.WeekNum) %>%
...
current_last_wait_site_all_ptType <- time_to_appt_all %>%
    ...
    filter(Appt.Made.Year %in% unique(past_4_wks$Appt.Year)) %>%
    filter(Appt.Made.WeekNum %in% unique(past_4_wks$Appt.WeekNum)) %>%
    group_by(Appt.Year, Appt.WeekNum, New.PT3) %>%
...
cols <- c("Appt.Year","Appt.WeekNum","Established","New")
...
current_last_wait_dept_all_ptType <- time_to_appt_all %>%
  # filter(Appt.Year == todays_year) %>%
  filter(Appt.WeekNum == todays_week) %>%
  group_by(Department, Appt.Year, Appt.WeekNum, New.PT3) %>%
  summarise(days = median(Wait.Time)) %>%
  pivot_wider(
    names_from = New.PT3,
    values_from = days
...
current_last_wait_dept_combined <- current_last_wait_dept_combined %>%
  select(Department, Visit.Method, New.PT3, days) %>%
  arrange(Department) %>%
  arrange(factor(Visit.Method, levels = c("Total","IN PERSON","TELEHEALTH")))
...
current_last_noShow_dept <- current_last_noShow_dept %>%
  # arrange(Appt.WeekNum) %>%
  select(Campus.Specialty, Department, Visit.Method, Arrived, `No Show`, noShow_rate)
...
# hh_site_role <- hh_site %>%
#   mutate(perc_comp = percent(Yes/total_obs,0)) %>%
#   select(Appt.DateYear, role, perc_comp, total_obs, order)
# 
# hh_site_all <- hh_site %>%
...
#   mutate(perc_comp =percent(Yes/total_obs,0),
#          role = "All") %>%
#   select(Appt.DateYear, order, perc_comp, total_obs) %>%
#   pivot_wider(
#               names_from = order,
#               values_from = c(perc_comp,total_obs)) %>%
#   select(Appt.DateYear, `perc_comp_a-bef`, `total_obs_a-bef`, `perc_comp_b-aft`, `total_obs_b-aft`)
# 
...
#   mutate(perc_comp = percent(Yes/total_obs,0)) %>%
#   select(unit, role, perc_comp, total_obs, order)
# 
# hh_dept_all <- hh_dept %>%
#   group_by(unit, order) %>%
#   summarise(total_obs = sum(total_obs),
...
#   mutate(perc_comp =percent(Yes/total_obs,0),
#          role = "All") %>%
#   select(unit, order, perc_comp, total_obs) %>%
#   pivot_wider(
#               names_from = order,
#               values_from = c(perc_comp,total_obs)) %>%
